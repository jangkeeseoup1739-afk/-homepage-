...
| 음력 생일을 넣으면 사주가 통째로 틀림 | KASI 기준 음력표를 코드에 담아 양력으로 환산 후 계산 (윤달 포함) |
| 없는 날짜(2월 31일, 29일까지인 달의 30일)를 고를 수 있음 | 고른 연·월에 실제로 있는 날짜만 선택지에 표시 |
...
- 음력 → 양력 환산이 KASI 기준 **54,779개 날짜에서 100% 일치**하고,
  음력으로 넣은 날짜와 같은 날을 양력으로 넣은 날짜의 사주가 서로 동일
...
음력은 천문 관측(삭)으로 정해져 수식으로 계산할 수 없고, 한국천문연구원이
확정한 값을 표로 들고 있어야 합니다. 코드 안 `LUNAR_TABLE` 이 그 표이며
(150년치를 750자로 압축), 만든 과정은 이렇습니다.

1. KASI 자료 기반 `korean_lunar_calendar` 에서 1900~2049년의 윤달 위치와
   각 달의 크기(29/30일)를 뽑아냅니다.
2. 각 해의 총일수 합이 다음 해 설날까지의 간격과 맞는지 확인합니다.
3. 1912~2049년 음력 초하루 1,718개가 전부 한국 표준시(UTC+9) 기준 삭이 든
   날과 같은지 천문 계산(`pyephem`)으로 교차 검증합니다.
4. 마지막으로 음력 날짜 54,779개를 전부 환산해 KASI 값과 대조합니다.

**한국 음력은 중국 음력과 다릅니다.** 삭의 시각을 각자의 표준시로 판정하기
때문인데, 1988년 설날이 한국은 2월 18일, 중국은 2월 17일인 것이 그 예입니다
(삭: 1988-02-17 15:54 UTC → 한국 02-18 00:54, 중국 02-17 23:54).
인터넷에 흔한 중국 음력표를 가져다 쓰면 이런 해에 사주가 하루 어긋납니다.
...
- **음력 입력은 한국천문연구원(KASI) 기준으로 양력 환산됩니다.** 윤달도
  지원하며, 고른 해에 실제로 윤달이 있을 때만 선택지가 나타납니다.
  지원 범위는 음력 1900~2049년입니다.
