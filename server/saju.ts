...
export function generateLocalFallbackInterpretation(saju: any) {
  ...
  const adviceByDominant: Record<string, any> = {
  ...
  const lucky = adviceByDominant[dominant] || adviceByDominant.wood;

  return {
    summary: `${saju?.name}님은 ${saju?.dayMaster}의 기운을 타고나 강인한 주체성과 깊은 내면의 잠재력을 지니셨습니다. 사주 전체에서 ${dominant === 'wood' ? '성장과 진취성' : dominant === 'fire' ? '열정과 표현력' : dominant === 'earth' ? '신뢰와 중재력' : dominant === 'metal' ? '결단과 완성도' : '지혜와 융통성'}이 돋보이며, 인생의 굴곡을 기회로 바꾸는 저력을 갖고 있습니다.`,
